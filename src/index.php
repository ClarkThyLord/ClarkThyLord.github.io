<!DOCTYPE html>
<html lang="en">
	<head>
		<link rel="icon" href="./assets/potential.svg" />
		<title>Potential - Home</title>

		<meta charset="utf-8" />
		<meta name="viewport" content="width=device-width, initial-scale=1.0" />
		<meta name="description" content="Potential, unlock that which is unknown!" />
		<meta name="keywords" content="www,potential" />

		<!-- CSS -->
    <link rel="stylesheet" href="./css/libs/foundation.min.css">
		<link rel="stylesheet" href="./css/libs/foundation.css">
		<link rel="stylesheet" href="./css/libs/gui-potential.css">

    <link rel="stylesheet" href="./css/common.css">

		<style>
		</style>
	</head>
	<body>

		<nav class="gui menu horizontal">
			<div class="section left">
				<div>
					<img src="./assets/potential.svg" class="icon relative small" />
					<span>Potential</span>
				</div>
				<div>
					<span>
						Work
					</span>
					<ul>
						<li>
							Art
						</li>
						<li>
							Design
						</li>
						<li>
							Literature
						</li>
						<hr />
						<li>
							View more...
						</li>
					</ul>
				</div>
				<div>
					Projects
					<ul>
						<li>
							WS-potential
						</li>
						<hr />
						<li>
							View more...
						</li>
					</ul>
				</div>
			</div>

			<div class="section center">

			</div>

			<div class="section right">
				<div>
					About
				</div>
			</div>
		</nav>

		<!-- JS -->
		<script src="./js/libs/jquery-3.3.1.min.js"></script>
		<script src="./js/libs/gui-potential.js"></script>

    <script src="./js/common.js"></script>

		<script>
		</script>
	</body>
</html>
