...
$(document).ready(function() {
  ...
  if (panels.length > 0) {
    ...
    for (let panel of panels) {
      html += "<li onclick=\"setupPanel('2D', '" + panel.replace(" ", "") + "');\"> <a> " + panel + " </a></li>";
    }
    html += "</ul>";
    $("#panels").append(html);
  }
...
function resize() {
  ...
  stage.canvas.width = window.innerWidth - offset.left;
  stage.canvas.height = window.innerHeight - offset.top;
